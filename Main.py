         ...
         for j in range(1000):
            try:
               if response.json()["liveData"]["plays"]["allPlays"][j]["result"]["event"] == "Goal":
                  ...
                  writer.writerow(["Goal", goalx, goaly, gshot_type, desc, distance(goalx,goaly), gteam, home_team, gperiod , YEAR,  (i,j)])
               if response.json()["liveData"]["plays"]["allPlays"][j]["result"]["event"] == "Shot":
                  ...
                  writer.writerow(["non goal shot", nongoalx, nongoaly, ngshot_type, ngdesc, distance(nongoalx,nongoaly), ngteam, home_team, ngperiod, (i, j)])
            except IndexError:
               break
      except KeyError:
         continue
