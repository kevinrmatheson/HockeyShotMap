...
"""Prototype HockeyDB scraper.

This module is not part of the active NHL Stats API pipeline.
It remains here for future work and has known unresolved implementation gaps.
"""

# TO DO: need to create a csv to use for the entire data spit out for use with these functions
def league_year_scrape(league, year, file_name=None):
    # Intended high-level flow: league page -> season page -> team page -> player page.
    ...
    # Map short league codes to HockeyDB display names.
    ...
    # Downstream scrape step only runs if an output target was provided.
    if file_name is not None:
        for index, team in enumerate(teams):
            team_scrape(team_links[index], file_name)

def team_scrape(link, file_name):
    # Intended to extract player table rows for one team page.
...
def player_scrape(link, file_name):
    # Placeholder for player bio scraping (height, weight, DOB).
    pass

if __name__ == "__main__":
    print("Scraper.py is a prototype and is not wired into the active pipeline.")